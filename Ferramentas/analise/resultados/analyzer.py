...
if __name__ == "__main__":
    ...
    # ⚠️ Esta é a parte corrigida, que estava com erro de sintaxe.
    def safe_run(cmd, out_path=None, timeout=None):
        try:
            # Nota: usamos check=False para não travar em erros de comando (Nmap, etc.)
            ...
            # Sintaxe corrigida (bloco 'if' claro):
            if out_path: 
                with open(out_path, "w") as f: 
                    f.write(proc.stdout.decode('utf-8', errors='ignore')) # Adicionado .decode() para segurança
